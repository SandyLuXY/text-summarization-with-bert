...
class Trainer(object):
    def __init__(self, args, rank=0):
        ...
        self.train_data = self.load_data(args.train_file, 'train.pt', is_test=False) #train_file : train.csv
        self.dev_data = self.load_data(args.dev_file, 'dev.pt', is_test=False) # dev.csv
    ...
    def load_data(self, file_name, loader_name, is_test=False):
        ...
        if os.path.exists(loader_file):
            ...
            if not is_test:
                trg_data = []
            src_df=pd.read_csv(os.path.join(self.dataset_dir, file_name), header=None, sep='\t')
            for l in range(len(src_df)):
                src=src_df.iloc[l,0] #news article
                #print("\nsrc: ", src)
                src_data.append(src)
                if not is_test:
                    trg=src_df.iloc[l,1]
    ...
    def make_dataloader(self, rank, data_dict, batch_size, shuffle=True):
        if "trg_input_ids" in data_dict: #depends of is_test
    ...
    def train(self):
                                    #(self, rank, data_dict, batch_size, shuffle=True)
    ...
    def rouge_eval(self, ref_dir, dec_dir):
        r = pyrouge.Rouge155()
    ...
    def rouge_log(self, results_dict, dir_to_write):
        log_str = ""
        for x in ["1", "2", "l"]:
            log_str += "\nROUGE-%s:\n" % x
